...
1. Upon the call to run CSV file data through the ZandrEA instance, the following applies:
   ...
   - The current data driving routine, because it executes in a terminal of the local host computer and accesses a CSV file directly reachable from its directory, must initiate a fairly complex call stack to pass data values to the Runtime API. This call stack includes Docker bridge network calls upon the `ea-rest` Docker container using http requests, as well as intervening code in JavaScript and C++ source files such as `[root]/EAd/handler.hpp` and `handler.cpp`. Development of Future Code B requires any new data driving routine to also follow that call stack.
