...
1. Build a local instance of ZandrEA from your repo by a command line interface (CLI) call of `make docker-up` at a terminal active in the root directory. The argument `docker-up` is a target within the root Makefile.
   ...
   - Also, there will be a volume ZandrEA creates for its knowledge base (KB).
...
1. Clicking for  "Subjects" in the pull-down will list the equipment pieces in the IBAL equipment that the demo has under AFDD surveillance. Because no building automation system (BAS) data has yet run through this surveillance, the GUI shows little else. However, its connection to the ZandrEA container driving it is confirmed by a green double-arrow, seen to the right of the bell-shaped icon used to access an "alerts queue".
## C. Next, run a file containing IBAL data through your ZandrEA instance:
1. Note the location of data files in .csv format at `[repo root]/EAd/tests/testdata/`. The file used there will be `ibal_demo_250709_si.csv`. It contains approximately 24 hours of data sampled from two AHU and four VAV boxes in the IBAL, at 60 seconds per sample.
...
```python3 ./EAd/tests/ead-push-date-time-ahu-vav-from-csv.py -t now -s 60 ./EAd/tests/testdata/ibal_demo_250709_si.csv```<br><br>
...
Opening localhost with the browser again will show the GUI after the demo is run.  Refer to Section 6.4 of the ZandrEA primer for guidance on navigating the GUI. Basically, you can:
...
```python3 ./EAd/tests/ead-push-date-time-ahu-vav-from-csv.py -i 1 -t now -s 60 ./EAd/tests/testdata/ibal_demo_250709_si.csv```<br><br>
The added flag and value ` -i 1` after the filename slows execution to one second per file row.
1. Go to the browser GUI and see AFDD states proceeding in time as data passes. The various GUI elements still function as was seen before. This is not as slow, however, as a run made at the real-time rate, which would occur for ` -i 60`.  Other features of the Python "data driver" script can be found by calling its help feature from the terminal command line.
...
1. Upon the call to run CSV file data through the ZandrEA instance, the following applies:
   ...
   - The current data driver routine, because it executes in a terminal of the local host computer and accesses a CSV file directly reachable from its directory, initiates a fairly complex call stack to pass data values to the Runtime API. This call stack includes Docker bridge network calls upon the `ea-rest` Docker container using http requests, as well as intervening code in JavaScript and C++ source files such as `[root]/EAd/handler.hpp` and `handler.cpp`. Development of Future Code B requires any new data driving routine to also follow that call stack.
