...
ahudata = [2.5, 85.0, 0.5, 79.0, 74.0, 0.4, 58.0, 57.5, 1.0, 0.0, 4500.0]
...
NcVAV = len(pointsVAV)
NcAHU = len(pointsAHU)
Ns = len(data[0])       # number of consecutive data "samples" in test
...
def GetSubjects():  # TODO: only return VAV subjects!
    ...
    for skey in subjects:
        ...
        if s['points'] == pointsVAV:
            VAVsubjects.append(skey)
            print("Subject {} marked as VAV".format(skey))
        elif s['points'] == pointsAHU:
            AHUsubjects.append(skey)    #TODO: should check AHU points, but currently broken in DLL
            print("Subject {} marked as AHU".format(skey))
        else:
            print("WARNING: skipping subject {}: unrecognized type with points {}".format(skey, s['points']))
...
def SampleTimeStep(timestamp, samples):
    """
    Push timestamp, values to ALL subjects, and singlestep
    NOTE: AHU has been added! For now just push fixed values for every AHU
    """
    global subjects, baseurl, headers
    url = "{}{}".format(baseurl, "/ctrl/sampletimestep")
    body = {'time': timestamp, 'values_by_subject': samples}
    response = requests.put(url, headers=headers, json=body)
    assert response.status_code >= 200 and response.status_code <= 299
    #print("SetCurrentSamples() succeeded")
...
for ii in range(0, 1):  # effectively: run once
    for s in range(Ns): # Ns == number of samples
        j = j + 1

        vavsample = [data[c][s] for c in range(NcVAV)]  # Nc == number of channels of data
        ahusample = ahudata;
        ...
        for s in subjects:
            if s in VAVsubjects:
                values.append({'subject': s, 'values': vavsample})
            elif s in AHUsubjects:
                values.append({'subject': s, 'values': ahusample})
        SampleTimeStep(tdsNow, values)  # Consolidates SetCurrentSamples, SetCurrentTM, and SingleStepAFDD
